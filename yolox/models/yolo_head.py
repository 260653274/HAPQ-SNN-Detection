...
class YOLOXHead(nn.Module):
    ...
    def get_assignments(
        ...
        with torch.amp.autocast('cuda', enabled=False):
            ...
            if not torch.isfinite(cls_preds_).all():
                logger.warning(f"NaN or Inf found in cls_preds_ at batch {batch_idx}")
                cls_preds_ = torch.nan_to_num(cls_preds_, nan=0.0, posinf=1.0, neginf=0.0)
            
            cls_preds_ = cls_preds_.clamp(0.0, 1.0)
    ...
    def simota_matching(self, cost, pair_wise_ious, gt_classes, num_gt, fg_mask):
        ...
        for gt_idx in range(num_gt):
            ...
            # Ensure k is not larger than the number of candidates (pair_wise_ious.size(1))
            k = int(min(k, cost.size(1)))
